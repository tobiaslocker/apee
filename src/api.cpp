...
api::AbstractRequestHandler::~AbstractRequestHandler() {}
...
api::HttpConnection::HttpConnection(
    tcp::socket socket,
    std::shared_ptr<api::AbstractRequestHandler> request_handler)
    : m_socket(std::move(socket)),
      m_request_handler(std::move(request_handler)) {}

void api::HttpConnection::start() {
  BOOST_LOG_CHANNEL_SEV(m_lg, m_channel, debug) << "Started";
  read_request();
  check_deadline();
}

void api::HttpConnection::read_request() {
...
void api::HttpConnection::process_request() {
...
void api::HttpConnection::handle_options_request() {
...
void api::HttpConnection::handle_post_request() {
  BOOST_LOG_CHANNEL_SEV(m_lg, m_channel, info) << "POST " << m_request.target();
  m_response.result(http::status::ok);
  m_response.set(http::field::server, "Beast");

  if (m_request_handler) {
    auto response = m_request_handler->on_post_request(m_request.target(),
                                                       m_request.body());
    if (!response.is_null()) {
...
void api::HttpConnection::handle_get_request() {
  BOOST_LOG_CHANNEL_SEV(m_lg, m_channel, info) << "GET " << m_request.target();
  m_response.result(http::status::ok);
  m_response.set(http::field::server, "Beast");

  if (m_request_handler) {
    auto response = m_request_handler->on_get_request(m_request.target());
    m_response.set(http::field::content_type, "application/json");
    m_response.set(http::field::access_control_allow_origin, "*");
    boost::beast::ostream(m_response.body()) << response;
...
void api::HttpConnection::handle_target_not_found() {
...
void api::HttpConnection::write_response() {
...
void api::HttpConnection::check_deadline() {
...
api::HttpServer::HttpServer(const api::Config &config)
    : m_acceptor{m_ioc, {config.address(), config.port()}}, m_socket{m_ioc} {}

void api::HttpServer::add_connection() {
  BOOST_LOG_CHANNEL_SEV(m_lg, m_channel, debug) << "Accepting requests";
  m_acceptor.async_accept(m_socket, [&](boost::beast::error_code ec) {
    if (!ec) {
      std::make_shared<HttpConnection>(std::move(m_socket), m_request_handler)
          ->start();
    } else {
...
void api::HttpServer::install_request_handler(
    std::shared_ptr<api::AbstractRequestHandler> handler) {
  BOOST_LOG_CHANNEL_SEV(m_lg, m_channel, debug) << "Installing request handler";
